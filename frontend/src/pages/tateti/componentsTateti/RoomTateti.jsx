...
import { URL as BASE_URL, URL_FRONT } from "../../../common/constants";

const urlJoin = `${URL_FRONT}/tateti/rooms/join/`;
const url = `${BASE_URL}/tateti/rooms/`;
const RoomTateti = (props) => {
  ...
  async function giveUp() {
    ...
    try {
      await rest.delete(`${url}${props.roomID}`, giveUpInfo);
    } catch (error) {
      showAlertOnScreen(true, error.message);
      return;
    }
  ...
  return (
    <div>
      <div className="container">
        ...
        <h1>Sala del TaTeTi</h1>
        <br />
        <h3>
          Usted está en la sala{" "}
          {props.roomID == null ? "No asignada" : props.roomID}
        </h3>
        <br />
        <ShowCopyURL url={urlJoin} roomID={props.roomID} />
        <br />
        <p>
          {" "}
          Recuerde, empiezan las X.
          <br />
          Si quiere ser círculo, no llore y pídale a su amigue que inicie la
          sala por usted.
          <br />
          La victoria puede parecer obvia sin poner el último símbolo, pero se
          necesita que quede asentado en el tablero para determinar al ganador.{" "}
          <br />
        </p>
        <br />
